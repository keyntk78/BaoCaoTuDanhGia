            ...
            <form action="{{ route('baocao.update', ['id' => $baoCao->id]) }}" method="POST">
                ...
                <div class="form-group">
                    ...
                    <select class="form-select form-control {{ $errors->has('nganh_id') ? 'is-invalid' : '' }}"
                        ...
                        <option {{ old('nganh_id', $baoCao->nganh_id) == '' ? 'selected' : '' }}>Chọn ngành</option>
                        @foreach ($nganhs as $item)
                            <option value="{{ $item->id }}" {{ old('nganh_id', $baoCao->nganh_id) == $item->id ? 'selected' : '' }}>{{ $item->ten }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <select class="form-select form-control {{ $errors->has('tieuChi_id') ? 'is-invalid' : '' }}"
                        ...
                        <option {{ old('tieuChi_id', $baoCao->tieuChi_id) == '' ? 'selected' : '' }}>Chọn tiêu chí</option>
                        @foreach ($tieuChis as $item)
                            <option value="{{ $item->id }}" {{ old('tieuChi_id', $baoCao->tieuChi_id) == $item->id ? 'selected' : '' }}>Tiêu chí số {{ $item->tieuChuan->stt }}.{{ $item->stt }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <textarea class="form-control tiny-editor" id="moTa" name="moTa"
                        rows="8">{{ old('moTa', $baoCao->moTa) }}</textarea>
                </div>
                <div class="form-group">
                ...
                <div class="form-group">
                    ...
                    <input type="number" class="form-control" id="diemTDG"
                        name="diemTDG" value="{{ old('diemTDG', $baoCao->diemTDG) }}">
                </div>
