@extends('layouts.index', ['title' => 'Thùng rác bộ tiêu chuẩn'])
    ...
    <div class="card shadow mb-4">
        @if (count($boTieuChuans) > 0)
            <div class="trash-action card-header py-3 d-flex flex-row align-items-center">
                <a href="#" class="btn btn-primary btn-icon-split btn-restore-all mr-3"
                    data-url="{{ route('botieuchuan.restore-all') }}"
                    data-redirect="{{ route('botieuchuan.index') }}">
                ...
                <a href="#" class="btn btn-danger btn-icon-split btn-force-delete-all"
                    data-url="{{ route('botieuchuan.force-destroy-all') }}"
                    data-redirect="{{ route('botieuchuan.index') }}">
                    <span class="icon text-white-50">
        ...
        <div class="card-body">
            <div class="table-responsive">
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <a href="#" class="btn btn-primary btn-restore"
                                            data-url="{{ route('botieuchuan.restore') }}" data-id="{{ $item->id }}">Phục
                                            hồi</a>
                                        <a href="#" class="btn btn-danger btn-force-delete"
                                            data-url="{{ route('botieuchuan.force-destroy') }}"
                ...
                {{ $boTieuChuans->render('pagination::bootstrap-4') }}
            </div>
        </div>
    </div>
@endsection
